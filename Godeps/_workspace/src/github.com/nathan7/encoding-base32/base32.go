...
const encodeMin = "0123456789abcdefghijklmnopqrstuv"

// NewEncoding returns a new Encoding defined by the given alphabet,
// which must be a 32-byte string.
func NewEncoding(encoder string, padding byte) *Encoding {
	e := new(Encoding)
	e.encode = encoder
...
func (enc *Encoding) decode(dst, src []byte) (n int, end bool, err error) {
	...
	for len(src) > 0 && !end {
		...
		for j := 0; j < 8; {
			...
			in := src[0]
			src = src[1:]
			if in == enc.padding && j >= 2 && len(src) < 8 {
				...
				for k := 0; k < 8-1-j; k++ {
					if len(src) > k && src[k] != enc.padding {
						// incorrect padding
